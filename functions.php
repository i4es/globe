...
// Globecore theme settings
include_once get_stylesheet_directory() . '/inc/globecore-settings.php';
//Globecore multi-sites info
include_once get_stylesheet_directory() . '/inc/globecore-sites-info.php';
...
//		wp_enqueue_script( 'jarallax', get_template_directory_uri() . '/js/plugins/jarallax.min.js', null, '1.10.7', true );
		wp_enqueue_script( 'snazzy-info-window', get_template_directory_uri() . '/js/plugins/snazzy-info-window.min.js', null, '1.1.1', true );
		// landings
		if ( is_single('6751') ) {
			wp_enqueue_style( 'landing-usa', get_template_directory_uri() . '/css/landings/landing-usa.css');
		}

		wp_enqueue_script( 'ajax-filter', get_template_directory_uri() . '/js/ajax-filter.js', array( 'jquery' ), true );
		wp_enqueue_script( 'google.maps.api', 'https://maps.googleapis.com/maps/api/js?key=' . (get_theme_mod( 'google_maps_api' ) ?: 'AIzaSyDDi4ZYpDVqG0e6V3Cc4YsMqirxYo4SRGs') . '&v=3.exp', null, null, true );
...
function gc_filter_function(){
	...
	if(isset($formData['cats']) && !empty($formData['cats'])){
		$filterArgs['category__in'] = $_POST['cats'];
	}

	if(isset($formData['gc_year']) && !empty($formData['gc_year']))
		$filterArgs['date_query'] = array(
			array(
				'year' => $formData['gc_year'],
			),
		);

	$cat_ids = implode(",", $formData['cats']);

	$cats = " category='$cat_ids'";
	$year = " year='".$formData['gc_year']."'";
	$date = " order='".$formData['gc_date']."'";

	echo do_shortcode( '[ajax_load_more posts_per_page="7" images_loaded="true"  scroll="false" button_label="More posts" ' .$cats . $year . $date. ' ]' );

	die();
}

add_action('wp_ajax_gcfilter', 'gc_filter_function');
add_action('wp_ajax_nopriv_gcfilter', 'gc_filter_function');

add_action( 'wp_enqueue_scripts', 'gc_ajax_data', 99 );
function gc_ajax_data(){
	wp_localize_script( 'ajax-filter', 'ajaxUrl',
		admin_url('admin-ajax.php')
	);
}

// Off contact form 7 recap on home page
...
function google_counter () {
    ...
    <script defer>
        ...
        ga('create', '<?php $g_analitic = globecore_get(); print_r ($g_analitic[0]);?>', 'auto' , {'name': 'CurrentMultiSite'});  // current website
        ga('send', 'pageview');
